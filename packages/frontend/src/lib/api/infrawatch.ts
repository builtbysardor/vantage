...
export const infrawatch = {
  ...
  containers: () => req<{ total: number; containers: unknown[] }>("/api/containers").then((r) => r.containers),
  historyMetrics: (hours = 24) => req<{ snapshots: unknown[] }>(`/api/history/metrics?hours=${hours}`).then((r) => r.snapshots),
  historyAlerts: (hours = 48) => req<{ alerts: unknown[] }>(`/api/history/alerts?hours=${hours}`).then((r) => r.alerts),
  analytics: (hours = 24) => req<unknown>(`/api/analytics/summary?hours=${hours}`),
  anomalies: (hours = 24) => req<{ events: unknown[] }>(`/api/anomalies?hours=${hours}`).then((r) => r.events),
